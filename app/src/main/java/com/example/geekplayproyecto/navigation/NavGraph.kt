...
import com.example.geekplayproyecto.ui.screen.NotificationsScreen
...
fun AppNavGraph(
    ...
    val banNotificationRepo = remember { ServiceLocator.provideBanNotificationRepository(context) }

    val userPrefs = remember { UserPreferences(context) }
    val isLoggedIn by userPrefs.isLoggedIn.collectAsStateWithLifecycle(false)

    // ✅ CAMBIO: Observamos el userId en lugar del email
    val currentUserId by userPrefs.userId.collectAsStateWithLifecycle(-1L)

    val currentUser by sessionViewModel.currentUser.collectAsStateWithLifecycle()

    // ✅ CAMBIO: Cargamos el usuario por ID
    LaunchedEffect(isLoggedIn, currentUserId) {
        if (isLoggedIn && currentUserId != -1L) {
            sessionViewModel.loadUser(currentUserId)
        } else {
    ...
    val goNotifications: () -> Unit = {
        scope.launch { drawerState.close() }
        safeNavigate(Route.Notifications.path)
    }
    ...
    val handleLogout: () -> Unit = {
        scope.launch {
            drawerState.close()
            userPrefs.clearLogin() // Usamos el método nuevo clearLogin
            sessionViewModel.clearSession()
        ...
        Scaffold(
            ...
            NavHost(
                ...
                composable(Route.Notifications.path) {
                    val factory = BanNotificationViewModelFactory(
                        repository = banNotificationRepo,
